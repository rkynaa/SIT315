...
matrixSize = atoi(argv[1]); // Read matrix size
numThreads = atoi(argv[2]); // Read threads

omp_set_num_threads(numThreads);  // Set threads per rank
...
numworkers = numtasks-1;
  ...
  if (processID == 0)
    ...
    printf("The OpenMP is running with %d threads.\n",numThreads);
  ...
  if (processID > 0)
     {
        ...
        // Primary difference of OpenMP is here
        #pragma omp parallel for shared(matrixA,matrixB,matrixC,numThreads) private(i,j,k)
